...
스택처럼 무른 중간층(OCA·PSA)이 끼면 두 면재가 서로 미끄러져 굽힘강성이 훨씬 낮다.
    ...
    f  = 1 − tanh(αL/2)/(αL/2)                      (합성도: 0=완전분리, 1=완전합성)
    EI_eff = EI_layered + f·(EI_full − EI_layered)

**두 극한이 정확히 닫힌다** — G_c→0 이면 f=0(각자 굼), G_c→∞ 이면 f=1(CLT 와 일치).
실측: UTG(30µm)/OCA(50µm)/UTG, OCA G=0.3 MPa, L=10mm 에서 f=46.8% 로
**CLT 가 굽힘강성을 2.03배 과대평가**한다. 스팬 의존이 강해 L=1mm 면 18.3배가 된다.
...
def composite_action(ea1: float, ei1: float, ea2: float, ei2: float,
    ...
    x = alpha_v * span / 2.0
    f = 1.0 - math.tanh(x) / x if x > 1e-300 else 0.0
    return {"composite_action": f, "EI_layered": ei_layered, "EI_full": ei_full,
            "EI_effective": ei_layered + f * delta_ei,
