...
- [x] ~~패턴층 직교이방~~ — v0.18.0 homogenize_patterned_layer. 등방 균질화로는 스택업 A/B 가
      바이트 동일했다(κ=1.85e-18). 이제 갈리고 안장 형상(warpage 4.6e-15 → 4.43mm)이 나온다.
      E1·ν12·α1 은 기존 도구와 정확 일치, 등방 극한도 일치
- [x] ~~접착 겹치기~~ — v0.19.0 assess_bonded_lap_joint (Volkersen). 겹침 4배에도 피크 불변,
      실효 비율 30%→7.5%. 단일 겹치기 peel 비보수를 W130
- [x] ~~노치 강도~~ — v0.19.0 assess_notched_strength. K_T 폐형해(등방 3.0000000000)와
      조건부 σ_OH 분리. d0 없으면 의도적으로 안 낸다
- [x] ~~준탄성 이완~~ — v0.19.0 solve_stress_relaxation. CLT 0.0% vs 부분합성 62.7% —
      순응층의 역할이 굽힘강성이 아니라 전단 결합임을 두 경로로 보인다
...
## 세션 4 — 적대 검증 2차 라운드 (2026-08-29, v0.20.0)

- [x] PC-01 부분합성 결합을 조화형으로 (선형은 중간 구간 2.4~6.1배 비보수)
- [x] NC-01/GATE-05 두께 배율이 횡전단 유연성을 함께 푼다 (3차식 고정 400회 이분법)
- [x] GATE-01/NC-02/GATE-02/GATE-06 안정성 게이트 (panel 침묵·주응력·횡전단 보정·M/h 규모)
- [x] NC-03/GATE-04 샌드위치 지배 모드 (코어 전단 독립 후보·면재 여유에서 코어 제외)
- [x] NC-05 자유단 박리 headline 에 개시 변형률·여유 + 여유<1 경고
- [x] NC-07 쪼갠 코어를 구간으로 병합 (기하평균 경계·직렬 조화평균 G)
- [x] GATE-03 열 구속 좌굴이 y축 압축도 푼다 (x↔y 교환)
- [x] GATE-07/08 변위 제어 체인 (applied_strain_x 스칼라·항복 게이트 이식)
- [x] GATE-09 흡습 균일도 측정 + 균일 delta_C 가정 명시
- [x] F1 포락선 이방비 0 나눗셈
- [x] DIF-01 작은 τ 에서 폐형해 분기 (500항 급수는 τ=1e-8 에서 97% 오차)
- [x] compute_buckling margin 에 보정 여유 병기 (검증 중 발견, 1.87배 낙관)
- [x] 테스트 506개 통과 · 문서 §19.25 · v0.20.0
- [ ] 배포 (HEAXHub 스캐너 → SIF → 게이트웨이 재집계)

## 세션 5 — 적대 검증 3차 라운드 (2026-08-29, v0.21.0)

- [x] PC2-01 부분합성을 N층 Newmark 정해로 일반화 (계면 여러 개)
- [x] PC2-02 EA·EI 규약을 1D 로 통일 (평행축 항등식 12자리)
- [x] PC2-02/07 코어 탐지 — 최외곽 오염 제거·각도 변환
- [x] PC2-04 이완 경로 g13 + RLX-01 bend_axis
- [x] PC2-01 계열 좌굴 — FSDT 전모드 최소 + 다중해상도 모드 탐색
- [x] SW-01/02/03 샌드위치 — 재료축 Tsai-Wu·x/y 방향·전단 합력
- [x] LJ-01/02 Volkersen 비대칭 항 + 오버플로
- [x] NT-01 노치 비 물리 상한
- [x] DIF-D1 탈습 프로파일 여집합
- [x] PC2-04/05/06, DOC-01 문서·경고 정합
- [x] 테스트 547개 · 결정론 10경로 확인 · 문서 §19.26 · v0.21.0
- [ ] 배포
