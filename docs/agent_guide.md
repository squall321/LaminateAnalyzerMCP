...
## 21. 동박 패턴층은 직교이방이다 (v0.18.0)

`homogenize_layer` 는 **등방** Voigt 라 `angle_deg` 가 무효다 — 패턴 방향만 다른 스택업
A/B 에 **바이트 동일한 답**을 준다(실측 κ=1.85e-18 양쪽). PCB 의 실린더/새들 형상은
그 방향 비대칭이 만드는 것이라 원리적으로 나올 수 없었다.

`homogenize_patterned_layer(components)` — 같은 입력 형식으로 **직교이방** 물성을 준다.
동박 60% 실측: E1/E2 = **8.55배**, α2/α1 = **2.24배**.

- **반환 material 을 넣고 `angle_deg` 를 트레이스 방향으로 설정**해야 한다. 그게 전부다.
  그러면 A/B 가 갈리고 안장 형상이 나온다(warpage 4.6e-15 → 4.43 mm).
- E1·ν12·α1 은 `homogenize_layer` 와 정확히 같다. 달라지는 건 E2·G12·α2 다.
- E2·G12 는 등응력 **하한**이라 보수적이다 — 실제는 E2 와 E1 사이다.
- 전 상이 같은 물성이면 등방 도구와 정확히 일치한다(안전한 대체재).

## 22. 이음·노치·이완 (v0.19.0)

**`assess_bonded_lap_joint`** — "겹침을 늘리면 강해진다"를 반박한다. 실측 Al 2mm 에서
25→100 mm 로 4배 늘려도 **τ_peak 가 133.6 MPa 로 그대로**이고 실효 비율만 30%→7.5% 로 떨어진다.
`saturation_overlap` 을 넘는 겹침은 무의미하다 — 접착층 두께·더 무른 접착제·테이퍼를 검토하라.
**`joint_type="single_lap"` 이면 결과가 비보수다** — 편심 굽힘의 peel 을 모델링하지 않는다.

**`assess_notched_strength`** — K_T 는 **폐형해**(등방 3.00, [0/90]s 4.92, UD 6.75)이고
σ_OH 는 **조건부**다. d0/a0 를 안 주면 σ_OH 를 **의도적으로 내지 않는다** — 그 상수는
시험 피팅이고 실측으로 d0=0.5~2.0mm 에서 σ_OH 가 234~368 MPa 로 갈린다.
`confidence` 필드로 두 값의 신뢰 등급을 구분해 보고할 것.

**`solve_stress_relaxation`** — 곡률을 고정한 채 점탄성층이 이완하는 것을 본다.
**두 경로를 모두 보라**: CLT 모멘트 이완은 0.0% 인데(순응층은 ABD 기여가 작다)
`span` 을 주면 부분합성 유효 굽힘강성이 **0.373배**로 무너진다(62.7% 감소).
순응층의 진짜 역할은 굽힘강성이 아니라 전단 결합이기 때문이다.
