...
## 9. 열탄성 확장 (v0.3.0, 계획서 §17.1)

- α 변환(공학 전단): αx=α₁m²+α₂n², αy=α₁n²+α₂m², αxy=2mn(α₁−α₂).
- N_th = ΔT ΣQ̄α t_k, M_th = ΔT ΣQ̄α z̄_k t_k → K[ε₀;κ]=[N_th;M_th] (자유 열변형).
- 잔류응력 σ = Q̄(ε₀+z̄κ−αΔT). ply 내 σ가 z 선형이므로 중앙값 힘 적분은 정확 → Σσt=0 불변식.
- 휨: w(x,y)=−½(κx x²+κy y²+κxy xy) (κ=−∂²w 규약 적분), coplanarity = 판 위 max−min.
- **Timoshenko 동등성**: 등두께 바이메탈 |κ|=24ΔαΔT/(h(14+n+1/n)), n=E₁/E₂. 두 층의 ν가
  같으면 등2축 판 문제는 E→E/(1−ν) 치환인데 식이 모듈러스 '비'에만 의존하므로 판=보 —
  CLT 결과와 rel 1e-6 일치(test_thermal).
- Voigt 균질화: E=Σf·E, α=Σf·E·α/Σf·E (등변형 힘 평형), 상한 모델.

## 10. 크랙 차폐 폐형해 (v0.3.0, 계획서 §17.2)

- 터널 G_ss = πσ²h/(4Ē) — 평면변형 중앙크랙(2a=h) 선단 G(=K²/Ē, K=σ√(πa))의 절반이
  정상상태 평균이라는 관계로 검증(test_fracture).
- σ_c = √(4ĒΓ/(πh)) (G_ss=Γ), δ_max = 2σh/Ē (타원 개구 프로파일 최대).
- Dundurs α=(Ē₁−Ē₂)/(Ē₁+Ē₂), β 표준식 — 부호로 차폐(α<0)/증폭(α>0) 경향만 보고,
  채널링 g(α,β) 수치 보정표는 미탑재(수기 이식 리스크 — 계획서 §17.2 정직성 원칙).
- He–Hutchinson: α≈0에서 Γi/Γℓ<1/4 ⇒ 계면 편향(크랙 저지).
- shear-lag ℓ=√(E_t h_t h_n/G_n); 점탄성 준탄성 근사로 ℓ(∞)/ℓ(0)=√(E₀/E∞).
